export default {
  'genre.scifi': 'מדע בדיוני',
  'genre.fantasy': 'פנטזיה',
  'genre.adventure': 'הרפתקאות',
  'genre.poetry': 'שירה',
  'genre.comedy': 'קומדיה',
  'genre.drama': 'דרמה',
  'genre.nonfiction': 'ספרות עיון',
  'genre.fiction': 'סיפורת',
  'genre.romance': 'רומנטיקה',
  'genre.satire': 'סאטירה',
  'genre.tragedy': 'טרגדיה',
  'genre.crime': 'פשע',
  'genre.historical': 'סיפור היסטורי',
  'genre.horror': 'אימה',
  'genre.mystery': 'מסתורין',
  'genre.thriller': 'מתח',
  'genre.western': 'מערב',
  'genre.memoir': 'ממור',
  'genre.biography': 'ביוגרפיה',
  'genre.steampunk': 'סטימפאנק',
  'genre.mythology': 'מיתולוגיה',
  'genre.fairytale': 'אגדה',
  'genre.superhero': 'סופר גיבור',
  'genre.children': 'ילדים',
  'genre.postapocalyptic': 'אפטר אפוקליפטי',
  'genre.political': 'מסמך פוליטי',
  'genre.history': 'History',
  'genre.isekai': 'איסקאי',
  'genre.scifi.description':
    "מדע בדיוני הוא ז'אנר ספרותי המחקר רעיונות מפוארים או עתידניים, ובדרך כלל משתלב בין אלמנטים מדעיים וטכנולוגיים. זה גם עוסק ברעיונות מדמינטייביים ועתידניים, כמו מדע מתקדם וטכנולוגיה, חקירת החלל, נסיעה בזמן, יחידות מקבילות וחייזרים. מדע בדיוני בדרך כלל עוסק בנושאים חברתיים, פוליטיים ופילוסופיים, משקפים ומתייחסים לחברה המודרנית. זה ניתן לסווג לתת-ז'אנרים כמו סייברפאנק, מדע בדיוני קשה, סיפורי דיסטופיה ואופרה חללית, בין היתר.",
  'genre.fantasy.description':
    "הפנטזיה היא ז'אנר ספרותי הכולל אלמנטים של קסם, אירועים על-טבעיים ויצורים מיתיים, ובדרך כלל מתרחש בעולמות מדומים. היא עשויה לכלול גיבורים במסעות, קרבות בין טוב לרע, וסיפורים על צמיחה. הפנטזיה יכולה להתחלק לתת-ז'אנרים כגון פנטזיה גבוהה, פנטזיה עירונית, פנטזיה אפלה וסיפורי יעדות, בין היתר. היא בדרך כלל משלבת נושאים של מוסריות ואתיקה, ומחקר את הטבע של טוב ורע.",
  'genre.adventure.description':
    "הרפתקאות הוא ז'אנר ספרותי המכיל מסעות מרתקים ולעיתים מסוכנים, הכוללים בדרך כלל פעולה פיזית ואתגרים. זה יכול לקרות בעולמות אמיתיים או מדומים ולעיתים משתתף בחקירה, חיפוש אוצרות או הישרדות מול הסיכונים. סיפורי הרפתקאות לעיתים קרובות מציגים גיבור או קבוצת גיבורים המתמודדים עם סתם ומתגברים על מכשולים, לעיתים עם עזרת חשיבה חכמה או יכולת פיזית. הז'אנר הזה ניתן לסווג לתת-ז'אנרים כמו סוושבאקלרים, סיפורי הישרדות וספרי מרגלים, בין היתר.",
  'genre.poetry.description':
    "שירה היא ז'אנר ספרותי המשתמש בשפה בצורה מאוד מובנית וביטויית כדי להעביר רגשות, רעיונות ודימויים. זה כמובן תבניות ריתמיות, התקנים של קול, ושפה מטאפורית כדי ליצור אפקט מובחן ועוצמתי. שירה יכולה לקחת צורות רבות, כולל סונטות, האיקוס, שירה חופשית ובלדות, בין היתר.",
  'genre.comedy.description':
    "קומדיה היא אחד מז'אנרים הספרותיים הקדומים ביותר המטרתו לצחוק ולקנות באמצעות שימוש בועט, חכמה וסאטירה. זה יכול לכלול דמויות, מצבים ואירועים מוגזמים, עם המטרה לגרום לצחוק ולשעשע מהקהל. קומדיה יכולה לקחת צורות רבות, כולל מחזות, רומאנים וסרטים. זה לעיתים עוסק בנורמות חברתיות, יחסים אנושיים והבדלים תרבותיים, ועשוי לספק הערות על נושאים עכשוויים בדרך מצחיקה.",
  'genre.drama.description':
    'דרמה היא אחת מצורות הספרות הקדומות ביותר המספרת סיפור באמצעות שיחה ופעולה, ולעיתים עם דמויות מורכבות וקונפליקטים. זה יכול לקחת צורות רבות, כולל מחזות, סרטים ותוכניות טלוויזיה. דרמה לעיתים עוסקת ביחסים וברגשות אנושיים, ויכולה להתמקד בנושאים של אהבה, בגידה וטרגדיה. היא עשויה גם לספק הערות חברתיות, שמתייחסות לנושאים וליחסים עכשוויים.',
  'genre.nonfiction.description':
    'Non-fiction is a literary genre that deals with real-life events, people, and experiences. It includes a wide range of writing, from memoirs and biographies to essays, journalism, and historical accounts. Non-fiction often aims to inform, educate, or persuade readers, presenting factual information in a clear and engaging way. It may explore social, political, scientific, or cultural issues, and may offer insights into the human condition.',
  'genre.fiction.description':
    'Fiction is a literary genre that involves invented stories, characters, and events, and is often distinguished from other genres such as science fiction or fantasy. It can take many forms, including novels, short stories, and novellas, and often explores the human experience through themes such as love, loss, and redemption. Unlike science fiction or fantasy, fiction typically takes place in a realistic or everyday setting, without incorporating supernatural or speculative elements. However, there can be some overlap between fiction and other genres, and many works can be categorized as multiple genres.',
  'genre.romance.description':
    'Romance is a literary genre that focuses on romantic love and relationships between characters. Romance often involves a central love story, featuring two characters who experience obstacles and conflicts that they must overcome to be together. It may explore themes such as passion, betrayal, and redemption, and may be categorized into sub-genres such as historical romance, contemporary romance, and paranormal romance, among others.',
  'genre.satire.description':
    'Satire is a literary genre that uses humor, irony, and exaggeration to criticize and expose the flaws and vices of society, politics, or individuals. Its primary purpose is to offer a critical message or underlying social commentary. In contrast, comedy is a genre that aims to entertain and amuse through humor, wit, and satire. While comedy may explore social norms and human relationships in a humorous way, it does not necessarily have a critical message or underlying social commentary. Satire is often more pointed and critical, while comedy is more lighthearted and playful.',
  'genre.tragedy.description':
    'Tragedy is one of the oldest literary genres that portrays the downfall or suffering of a tragic hero or heroine due to a fatal flaw or an unfortunate circumstance. It often evokes feelings of pity and fear in the reader or audience, and may explore themes such as fate, destiny, and the human condition. Tragedy aims to provide a cathartic experience for the audience and often has a moral or philosophical message.',
  'genre.crime.description':
    'Crime is a literary genre that revolves around the investigation, commission, and solving of crimes, often involving murder, theft, or other illegal activities. Crime stories often involve complex characters, motives, and relationships, and may explore themes such as justice, morality, and the human psyche. They aim to keep the reader or audience engaged and often provide a sense of closure when the crime is solved or justice is served.',
  'genre.historical.description':
    'Historical fiction is a literary genre that is set in the past and uses fictional characters and events to explore historical events, cultures, and customs. It often involves extensive research to ensure historical accuracy and may incorporate real-life figures or events. It aims to transport the reader or audience to a different time and place, while also providing insight into the human experience across history.',
  'genre.horror.description':
    'Horror is a literary genre that aims to create an intense feeling of fear, dread, or disgust in the reader or audience. It often involves supernatural or inexplicable elements, such as ghosts, monsters, or otherworldly entities, and may explore themes such as death, madness, and the unknown. Horror stories often aim to provoke a visceral response from the audience and may provide commentary on societal fears and anxieties.',
  'genre.mystery.description':
    'Mystery is a genre of fiction that revolves around a crime or puzzle that needs to be solved. Often featuring a detective or amateur sleuth as the protagonist, mysteries typically involve clues, suspects, and a final reveal or solution to the mystery. The genre can include elements of suspense, crime, and psychological thriller.',
  'genre.thriller.description':
    'Thriller is a genre of fiction that is designed to create a feeling of tension, excitement, and suspense in the reader. Typically featuring high stakes, danger, and intense action, thrillers can encompass a wide variety of sub-genres, such as espionage, legal, medical, or psychological thrillers. The goal of the genre is to keep the reader on the edge of their seat and engaged in the story.',
  'genre.western.description':
    'Western is a genre of fiction that typically takes place in the American Old West and features cowboys, outlaws, and lawmen as its central characters. The genre often explores themes of honor, justice, and individualism in a time when civilization was expanding into the frontier. Westerns can be further divided into sub-genres such as traditional, revisionist, or spaghetti westerns.',
  'genre.memoir.description':
    "Memoir is a non-fiction genre that tells the story of a person's life or a specific period of their life. Unlike an autobiography, which covers an entire life, memoirs focus on a particular aspect or event and often employ literary techniques to tell a compelling and engaging story. Memoirs can be written by anyone, from celebrities to ordinary people, and can cover a wide variety of topics and experiences.",
  'genre.biography.description':
    "Biography is a non-fiction genre that tells the story of a person's life. Unlike memoirs, which often focus on a specific period or aspect of a person's life, biographies aim to provide a comprehensive overview of a person's entire life. Biographies can be written about anyone, from historical figures to contemporary celebrities, and can be further classified by sub-genres such as political, cultural, or sports biographies.",
  'genre.steampunk.description':
    'Steampunk is a genre of fiction that combines elements of Victorian-era aesthetics, technology, and culture with futuristic or fantastical elements. Typically taking place in an alternate version of the 19th century, steampunk often features steam-powered machinery, clockwork contraptions, and airships. The genre can encompass a wide variety of sub-genres, such as steampunk mystery or steampunk romance.',
  'genre.mythology.description':
    "Mythology is a genre that explores the myths, legends, and stories of a particular culture or society. Often involving gods, goddesses, heroes, and monsters, mythology can provide insights into a culture's beliefs, values, and traditions. The genre can encompass a wide variety of sub-genres, such as Greek mythology, Norse mythology, or world mythology.",
  'genre.fairytale.description':
    'Fairy Tale is a genre of fiction that typically features magical elements, fantastic creatures, and enchanted settings. Often aimed at children, fairy tales typically involve a moral or lesson and can be traced back to oral traditions from around the world. The genre can encompass a wide variety of sub-genres, such as traditional fairy tales, modern retelling, or dark fairy tales.',
  'genre.superhero.description':
    'Superhero is a genre of fiction that typically features characters with extraordinary abilities or powers who use them to protect the world from evil or harm. Often wearing distinctive costumes or masks, superheroes can embody a variety of ideals and values, such as justice, compassion, or freedom. The genre can encompass a wide variety of sub-genres, such as superhero team-ups, origin stories, or alternate universes.',
  'genre.children.description':
    "Children's literature is a literary genre is specifically aimed at young readers, ranging from infants to pre-teens. These books can cover a diverse range of topics and themes, including adventure, fantasy, education, and moral lessons. The goal of children's literature is to entertain, engage, and educate young readers while fostering a love of reading.",
  'genre.postapocalyptic.description':
    "This sub-genre of science fiction or speculative fiction is set in a world or society after a catastrophic event has occurred, such as a global disaster, war, or pandemic. Post-apocalyptic stories often explore themes of survival, adaptation, and rebuilding, with characters fighting to survive against harsh environments and societal breakdowns. Often bleak and harrowing, these stories can also offer a glimmer of hope for humanity's future.",
  'genre.political.description':
    'Documents focusing on political matters, societal matters or for example artistic manifestos.',
  'genre.history.description':
    'Historical documents and other literary materials of historical significance or describing historical events.',
  'genre.isekai.description':
    "זהו תת-ז'אנר של רומנים חלומיים, מאנגה ואנימה יפניים שמתרחשים בעולם אחר או ביקום אחר. הדמות הראשית נשלחת או מתגלגלת מחדש לעולם שונה או ליקום אלטרנטיבי. הגיבור חייב לנווט ולהתאים את עצמו לתרבויות ולסביבות הייחודיות של העולם החדש, ולעיתים גם לרכוש יכולות חדשות ולפגוש בן זוגים ואויבים חדשים בדרך. סיפורים אלו יכולים להיות מצחיקים וקומיים או אפלים ומתוחים, אך תמיד מציעים גישה חדשה ומדהימה לנושאים המרקמים את ההרפתקאות הקלאסיות.",
}
