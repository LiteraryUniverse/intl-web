...
* Translation management migrated from Crowdin to self-hosted **Weblate**.
* All locale sources converted from `.js` (`export default { ... }`) to `.json`. No `.js` files remain.
* `<locale>/index.json` is now a committed artifact, regenerated by `scripts/buildIndex.mjs` (CI on every push to `master`).
* `crowdin.yml` removed.
