/**
 * @authors Jan Dvorak
 */
/* eslint-disable max-len */
export default {
  siteName: 'Literatura Universo',
  twitterHandle: '@lituniapp',
  'site.description':
    'Literary Universe estas ĉi-en-una kreiva kaj publikiga platformo por grandaj rakontaj universoj.',
  'site.keywords':
    'kreiva verkado, sciencfikcio, sci-fi, fantazio, verkado, publikigo, rakonto, rakontoj, universo, reta romano, retromano',
  'site.logoAlt': 'Literary Universe logo',
  mobileMenu: 'Mobile menu',
  'common.signin': 'Ensaluti',
  'common.signup': 'Registriĝo',
  'common.signout': 'Elsaluti',
  'common.dashboard': 'Dashboard',
  'common.discovery': 'Eltrovo',
  'common.library': 'Biblioteko',
  'common.library.own': 'Your Library',
  'common.workshop': 'Workshop',
  'common.pm': 'Mesaĝo',
  'common.settings': 'Agordoj',
  'common.options': 'Opcioj',
  'common.profile': 'Profilo',
  'common.blog': 'Blogo',
  'common.blogs': 'Blogs',
  'common.about': 'About us',
  'common.accessdenied': 'Aliro ne permesita',
  'common.accessdenied.msg':
    'You were not given permission to go here. If you should have access here, please contact the owner and tell them to double check the settings.',
  'common.loading': 'Loading...',
  'common.add': 'Aldoni',
  'common.save': 'Konservu',
  'common.email': 'Retpoŝto',
  'common.soon': 'Coming soon...',
  'common.search': 'Search',
  'common.search.do': 'Search',
  'common.searching': 'Searching...',
  'common.report': 'Raporti',
  'common.user': 'User',
  'common.users': 'Uzantoj',
  'common.showmore': 'Montru pli',
  'common.futurefeature.title': 'Future feature',
  'common.futurefeature.text':
    'We are very sorry, but this feature is not available at this time. We are working hard to make it available as soon as possible.',
  'common.nothingfound': 'Nenio trovita',
  'common.explore': 'Esplori',
  'common.by': 'per {author}',
  // keep the space at the end as username follows TODO: needs fixing
  'common.scratchpad': 'Scratchpad',
  'common.scratchpad.own': 'Your scratchpad',
  'common.news': 'News',
  'common.encyclopedia': 'Encyclopedia',
  'common.art': 'Arto',
  'common.story': 'Rakonto',
  'common.stories': 'Rakontoj',
  'common.universe': 'Universe',
  'common.universes': 'Universes',
  'common.collaborator': 'Collaborator',
  'common.collaborators': 'Collaborators',
  'common.translator': 'Translator',
  'common.translators': 'Translators',
  'common.betareader': 'Leganto de Beta-versioj',
  'common.betareaders': 'Beta readers',
  'common.post': 'Afiŝo',
  'common.statistics': 'Statistikoj',
  'common.lang.select': 'Select language',
  'common.submit': 'Sendi',
  'common.close': 'Fermi',
  'common.comment': 'Komentoj',
  'common.comments': 'Komentoj',
  404: 'Paĝo ne trovita',
  'search.users': 'Search for users',
  'search.user': 'Search for user',
  'common.help': 'Helpo',
  'common.or': 'aŭ',
  'common.account.create': 'Krei konton',
  'signup.username': 'Uzantnomo',
  'signup.email': 'Via retpoŝtadreso',
  'signup.password.label': 'Your password',
  'signup.password.repeat': 'Repeat your password',
  'common.password': 'Pasvorto',
  'signin.password.forgot': 'Forgot password?',
  'dashboard.whatsnew': 'What is new',
  'dashboard.blog': 'Your blog',
  'common.footer.tagline': 'Writing and reading complex universes made easy.',
  'common.footer.plans': 'Plans & Pricing',
  'common.legal.tos': 'Kondiĉoj de Servado',
  'common.legal.community': 'Komunumaj Gvidnormoj',
  'common.legal.privacy': 'Privateca Politiko',
  'common.legal.copyright': 'Copyright Agreement',
  'common.legal.versionDisplay': `{type, select,
    tos {Kondiĉoj de servado}
    community {Komunumaj gvidlinioj}
    privacy {Privateca politiko}
    copyright {Aŭtorrajta interkonsento}
    publishingAgreement {Afiŝa interkonsento}
    other {Interkonsento}
  } Versio {version} - {date, date}`,
  'common.legal.newVersionAvailable':
    'Nova versio ({newVersion}) de ĉi tiu dokumento enigos en vigo je {effectiveAt, date}.',
  'common.legal.disclaimer':
    '© {year} All rights reserved, all texts belong to their respective authors.',
  'premium.only.title': 'Not high enough membership rank',
  'premium.only.msg':
    'We apologize, your membership rank is not high enough to access this feature.',
  'premium.only.join':
    'We offer varieties of plans for everyone to get the most out of Literary Universe.',
  'premium.only.link': 'Check it out!',
  'universe.visit': 'Visit universe page',
  'common.fanmanager': 'Fana administranto',
  'common.fanarts': 'Fana arto',
  'common.fanfiction': 'Fana fikcio',
  'common.fansettings': 'Fana agordo',
  'story.chapter': 'Ĉapitro',
  'story.chapters': 'Ĉapitroj',
  'story.genre': 'Ĝenro',
  'story.genres': 'Ĝenroj',
  'story.read': 'Legi',
  'story.lastUpdate': 'Last updated on {date, date, medium} {date, time, short}.',
  'story.finishedAt': 'Finished on {date, date, medium}.',
  'story.comments.for': 'Comments for {story} {chapter}.',
  'story.cover.alt': 'Story cover image for {story}.',
  'story.note.fanfiction': 'NOTE: This story is a fan fiction on {universe} universe.',
  copyrights: 'Kopirajto',
  'common.challenges': 'Defioj',
  // for beta only
  'beta.notice':
    'Welcome to Literary Universe BETA. This is a public preview and as such the app is still work in progress. This means that you can still expect errors and unfinished features. If you encounter an error or something not working please let us know and please be patient while we get get everything in place.',
  'beta.thanks': 'Thank you for your support!',
  'scratchpad.about': 'Your notes and ideas accessible anywhere on the site.',
  'common.feedback': 'Prikomentado',
  'common.createdOn': 'Kreita je {date, date, long}',
  'common.createdOn.title': 'Kreita je',
  // account types
  admin: 'Administristo',
  'common.saved': 'Konservita!',
  'common.saving': 'Konservas...',
  'common.friends': 'Amikoj',
  'lang.orig': 'Original language',
  'lang.orig.plus': 'Original language - {language}',
  buy: 'Aĉeti',
  confirm: 'Konfirmi',
  cancel: 'Nuligi',
  processing: 'Processing...',
  'common.noData': 'Neniu datumo',
  'error.title': 'An error has occurred. We apologize for the inconvenience.',
  'common.offlineStatus': 'You are currently offline.',
  male: 'Viro',
  female: 'Ina',
  'common.legal.effectiveAt': 'Efektiva de {date, date, medium}',
  select: 'Bonvolu elekti',
  'common.statusFailed': 'Ne sukcesis konektiĝi al la servilo.',
  'common.statusConnecting': 'Provas konektiĝi al la servilo... ({count, number})',
  'slug.checking': 'Kontrolante...',
  'slug.failed': 'Tiu adreso jam ekzistas.',
  'slug.success': 'Tiu adreso estas havebla!',
  'notVerified.msg': `Via konto {option, select,
    email {retpoŝto}
    wallet {monujo}
    other {retpoŝto}
    } ne estas konfirmata.`,
  'notVerified.verify': `Bonvolu konfirmi vian {option, select,
    email {retpoŝto}
    wallet {monujo}
    other {retpoŝto}
    }.`,
  unknown: 'nekonata',
  'common.legal.guidelines': 'Komunumaj Gvidnormoj',
  'errorLoading.title': 'Eraro dum paĝokarado',
  'errorLoading.explained':
    'Okazis eraro dum la paĝo ŝargiĝis. Ni bedaŭras la maloportunecon. Bonvolu reprovi aliri la paĝon denove. Se la problemon daŭras, bonvolu sciigi nin.',
  'errorLoading.retry': 'Reprovi!',
  'errorLoading.takingLong': 'Tio daŭras longe...',
  'common.title': 'Titolo',
  'common.timelines': 'Tempolinioj',
  'common.legal.gdpr': 'GDPR',
  'common.legal.gdpr.long': 'Reguligo pri la Protektado de Datumoj',
  'editor.placeholder.link': 'Enigu URL-on...',
  'editor.placeholder.connection': 'Serĉu por enciklopedia artikolo...',
  'editor.saved.beta': 'Nova revizio kreita por Beta-legantoj!',
  'editor.saved.public': 'Nova versio publikigita!',
  goodbyeForever:
    'Ni bedaŭras, ke vi foriras. Ĉu estis io, kion ni faris? Bonvolu sciigi nin, se tio estas la kazo. Ajne, ni deziras al vi la plej bonan en viaj estontaj entreprenoj kaj esperas, ke ni renkontiĝos denove! Vivu longe kaj prosperu!',
  'goodbyeForever.feedback': 'Diru al ni, kion ni povas fari pli bone',
  'common.edit': 'Redakti',
  'common.publish': 'Publikigi',
  'common.publishAt': 'Dato kaj tempo de publikigo',
  'a11y.user.menu': 'Uzantmenuo',
  'a11y.navigation': 'Navigado',
  'dashboard.profile': 'Via profilo',
  'profile.website': 'Mia persona retejo',
  'story.notes.concept': 'Koncepto',
  'story.notes.settings': 'Rakonto-ĉirkaŭaĵo',
  'story.notes.theme': 'Etoso',
  'story.notes.impact': 'Dezirata efiko',
  'story.notes.outcome': 'Rezulto',
  'story.notes.audience': 'Cela publiko',
  'story.notes.pitch': 'Prezento',
  'story.notes.notes': 'Notoj',
  'story.authorsNotes': 'Notoj de la aŭtoro',
  'common.members': 'Membroj',
  'common.language': 'Lingvo',
  next: 'Sekva',
  previous: 'Antaŭa',
  'common.legal.tos.short': 'Terminoj',
  'common.legal.privacy.short': 'Privateco',
  'premium.increaseLimit': 'Konsideru ĝisdatigi por pliigi vian limon.',
  'common.sharing': 'Kunhavigado',
  'story.isbn': 'ISBN',
  'common.name': 'Nomo',
  'common.remove': 'Forigi',
  'common.time.milliseconds': 'Milisekundoj',
  'common.time.seconds': 'Sekundoj',
  'common.time.minutes': 'Minutoj',
  'common.time.hours': 'Horoj',
  'common.time.days': 'Tagoj',
  'common.time.weeks': 'Semajnoj',
  'common.time.months': 'Monatoj',
  'common.time.years': 'Jaroj',
  'common.monetization': 'Monetigo',
  'common.tab.monetizationOptions': 'Monetigo-opcioj',
  'common.pagination': 'Paĝinado',
  'common.forum': 'Forumo',
  'common.2faCode': 'Kodo de 2FA',
  'common.2faCodeIntro': 'Bonvolu enigi vian unufojan kodon generitan de via aŭtentigilo-aplikaĵo.',
  'common.2faCodeAria': 'Eniga formularo por kodo de 2FA',
  'common.flashnews': 'Flaŝnovaĵo',
  'common.defaultLanguage': 'Defaŭlta lingvo',
  'signin.web3.signMessage':
    'Bonvolu konfirmi, ke vi volas ensaluti en Literatura Universo per via monujo kiel aŭtentiga metodo. Kodo: {code}',
  'common.web3.confirmationCode': 'Kodo de konfirmo: {code}',
  'common.roadmap': 'Vojaĝplano',
  'common.luStatsIFrameOptOut': 'Statistikoj de Literatura Universo - malaktiviĝu',
  'common.image': 'Bildo',
  'common.home': 'Ĉefpaĝo',
  'common.administration': 'Administrado',
  'common.truncateAppend': '...',
  // What should be the symbol used to denote that the text continues when it was truncated
  'common.groups': 'Grupoj',
  'common.upload': 'Alŝuti',
  'common.backToDashboard': 'Reiri al panelo',
  'common.accept': 'Akcepti',
  'common.reject': 'Rifuzi',
  'common.events': 'Eventoj',
  'common.event': 'Evento',
  'common.all': 'Ĉiuj',
  'common.social': 'Socia',
  'common.discordCommunity': 'Komunumo de Discord',
  'common.account': 'Konto',
  'common.notifications': 'Sciigoj',
  'common.byDeclaration': 'De:',
  'common.done': 'Finita',
  recommended: 'Rekomendita',
  'select.typeToCreate': 'Komencu tajpi por krei elementon',
  'select.noSearchResults': 'Neniu rezulto por "{search}"',
  'select.removeOption': 'Forigu {option}',
  create: 'Krei',
  'banner.support':
    'Literary Universe estas subtenata de uzantoj kiel vi! Bonvolu konsideri abonadon al unu el niaj abonplanoj por resti sendependa kaj fari Literary Universe eĉ pli bona!',
  'banner.seePlans': 'Vizitu nian paĝon pri plano por pli da informoj.',
  'common.noAccountQuestion': 'Ĉu vi ne havas konton?',
  'common.haveAccountQuestion': 'Ĉu vi jam havas konton?',
  'common.socialLogin': 'Ensalutu per aliaj servoj',
  'common.viewMore': 'Vidu pli',
  'common.overview': 'Superrigardo',
  'common.loggingOut': 'Elsaluti',
  'common.thankYou': 'Dankon!',
  'common.seeYouAgain': 'Ĝis revido!',
  'email.hello': 'Saluton {username},',
  'common.back': 'Reen',
  'console.stop': 'Haltu!',
  'console.explain':
    'Tio estas retumilo-funkcio celata por programistoj. Se iu diris al vi kopii-kolii ion ĉi tien por ebligi funkciojn de Literatura Universo aŭ "haki" iun konton, tio estas trompo kaj donos al ili aliron al via Literatura Universo konto.',
  'common.coping': 'Kopii...',
} /* eslint-enable max-len */
