export default {
  'plans.desc': 'Description of plans and features of Literary Universe.',
  'plans.title': 'Get more out of Literary Universe!',
  'plans.text':
    'Literary Universe offers  basic functionality for free. If you are avid reader or a creator you might find our advance membership more to your liking.',
  'plans.faq.basicsPriced': 'Why has the free membership so few things?',
  'plans.faq.justification':
    'സാഹിത്യ യൂണിവേഴ്‌സിനെ ഞങ്ങളുടെ പ്രൈവസി അനുഭവങ്ങൾക്കും ആക്കാം നിങ്ങളുടെ ഡാറ്റ വിജ്ഞാപനങ്ങളും മറ്റു കമ്പനികളിലേക്ക് നൽകാനാക്കുന്നില്ലല്ലോ. അതായത് ഞങ്ങൾക്ക് ഒരുപാട് കാലം എടുക്കുകയും പ്രവർത്തിപ്പിക്കുകയും ചെയ്യുന്നതിന് നല്ലതായ പണം അടയ്ക്കുകയും ചെയ്യുന്നു. അതിനാൽ ഞങ്ങൾ സേവനം പുനഃനിർമ്മിക്കുകയും പുതുതായി വിളിക്കുന്നവരെയെല്ലാം അനുകൂലമായി അടുക്കുകയും ചെയ്യുന്നതിലൂടെ ലഭിക്കുന്ന പണം അടയ്ക്കുകയും ചെയ്യുന്നു. ഞങ്ങൾ എന്താണ് നിർമ്മിക്കുന്നത് നിങ്ങൾക്ക് ഇഷ്ടപ്പെടുന്നുവെങ്കിൽ, ദയവായി പേയ്മെന്റ് ഓപ്ഷനുകളിലൊന്നിനായി സൈൻ അപ്പ് ചെയ്യുക. നന്ദി!',
  'plans.limits': 'Limits',
  'plans.features': 'Features',
  'plans.account.user': 'Visitor',
  'plans.account.user.desc': 'Initial trial account, to try things out.',
  'plans.account.explorer': 'Explorer',
  'plans.account.explorer.desc': 'For hobbyist & friends.',
  'plans.account.adventurer': 'Adventurer',
  'plans.account.adventurer.desc': 'സൃഷ്ടിക്കുന്നവർക്കും ഞങ്ങളുടെ പിന്തുണക്കുകളിലേക്കുള്ളവർക്കും.',
  'plans.account.storyteller': 'Storyteller',
  'plans.account.storyteller.desc': 'For the most creative and dedicated.',
  'plans.unlimited': 'Unlimited',
  'plans.price': 'Price',
  'plans.price.free': 'FREE',
  'plans.price.monthly': 'a month*',
  'plans.price.annually': 'annually*',
  'plans.price.perMonth': '{price} / month*',
  'plans.price.perYear': '{price} / year*',
  'plans.development': 'In development',
  'plans.limits.encEntries': 'Encyclopedia entries',
  'plans.limits.maxCollaborators': 'Collaborators per project',
  'plans.limits.pmChats': 'Maximum # of recipients for private message',
  'plans.feature.creative': 'Create amazing universes and stories',
  'plans.feature.encCustomFields': 'Customizable encyclopedia fields on an entry',
  'plans.feature.encAdvCategories': 'More encyclopedia categories',
  'plans.feature.scratchpad': 'സ്ക്രാച്ച് പാട്',
  'plans.feature.library': 'ലൈബ്രറി',
  'plans.feature.blog': 'Personal & universe blogs',
  'plans.feature.customBlogTheme': 'Customizable blog themes',
  'plans.feature.advanceStats': 'Advanced statistics',
  'plans.feature.challenges': 'Writing challenges',
  'plans.feature.newFeaturesVote': 'Vote on new features',
  'plans.feature.newFeaturesSuggest': 'Propose new features',
  'plans.feature.accessToBeta': 'Access Beta features',
  'plans.feature.userGroups': 'User groups',
  'plans.feature.privateUserGroups': 'സ്വകാര്യമായിടയിൽ ഉപയോക്തൃ ഗ്രൂപ്പുകൾ സൃഷ്ടിക്കുകയും മറയ്ക്കുകയും ചെയ്യുക',
  'plans.feature.internalUniverseForums': 'യൂണിവേഴ്സുകളിലെ ആന്തരിക ഫോറം',
  'plans.feature.fictionalTime': 'കഥാസമയ സമയം',
  'plans.feature.forums': 'ബ്ലോഗുകളും യൂണിവേഴ്‌സ് പേജുകളിലുള്ള ഫോറങ്ങൾ',
  'plans.feature.timelines': 'യൂണിവേഴ്‌സ് ടൈംലൈനുകൾ',
  'plans.feature.webMonetization': 'വെബ് മോണെറ്റൈസേഷൻ',
  'plans.feature.storyMonetization': 'കഥ മോണിറ്റൈസേഷൻ',
  'plans.feature.fanClub': 'Fan Club',
  'plans.feature.translations': 'പരിഭാഷ സാധ്യതകൾ',
  'plans.feature.fanFiction': 'ഫാൻ ഫിക്ഷൻ എഴുതാൻ മറ്റൊരുവഴി',
  'plans.feature.universeFanArt': 'ഫാൻ ആർട്ട് മാനേജർ',
  'plans.feature.overviewNotes': 'കഥ അവലോകന കുറിപ്പുകൾ',
  'plans.feature.personalNotesStoriesUniverses': 'കഥകളും യൂണിവേഴ്‌സുകളും പ്രത്യേക കുറിപ്പുകൾ',
  'plans.feature.storyReviews': 'കഥ അവലോകനങ്ങൾ',
  'plans.feature.readingLists': 'റീഡിങ് ലിസ്റ്റുകൾ',
  'plans.feature.scenesManager': 'സീൻസ് മാനേജർ',
  'plans.feature.glossaries': 'പരിഭാഷ ഗ്ലോസറികൾ',
  'plans.feature.chatSupport': 'Chat support',
  'plans.currency.select': 'നാണയം തിരഞ്ഞെടുക്കുക',
  'plans.subscription.ownAlready': 'നിങ്ങൾക്ക് ഈ പദ്ധതി ഇപ്പോൾ ഉണ്ട്',
  'plans.subscription.title': 'സബ്സ്ക്രിപ്ഷൻ',
  'plans.see': 'ലഭ്യമായ സബ്സ്ക്രിപ്ഷൻ പ്ലാൻസ് കാണുക',
  'plans.current.none': 'നിങ്ങൾക്ക് ലിറ്റററി യൂണിവേഴ്‌സ് പിന്തുണയ്ക്കാനാവില്ല.',
  'plans.current.buying':
    'നിങ്ങളുടെ വാങ്ങിയ ഉത്തരവാദിത്തം പരിശോധിക്കുന്നതിന് കുറച്ചു മിനിറ്റുകൾ എടുക്കാൻ പടിക്കുന്നു. നിങ്ങളുടെ സഹായത്തിന് നന്ദിയുണ്ട്.',
  'plans.current.heading': 'നിലവിലുള്ള സബ്സ്ക്രിപ്ഷൻ പ്ലാൻ',
  'payment.methods.title': 'പണങ്ങൾ & ലഭിക്കൽ',
  'payment.methods.tabs': 'പണപ്പെട്ടതിന് പരിശോധനാ ഓപ്ഷനുകൾ',
  'payment.option.title': 'പരിശോധനാ ഓപ്ഷൻ',
  'payment.option.notFount': 'നിങ്ങൾക്ക് ഏതെങ്കിലും പരിശോധനാ ഓപ്ഷൻ സംരക്ഷിച്ചിട്ടില്ല.',
  'payment.option.create': 'പുതിയ പരിശോധനാ ഓപ്ഷൻ ചേർക്കുക',
  'payment.option.description':
    'ഇവിടെ നിന്നും നിങ്ങളുടെ സംരക്ഷിച്ച പരിശോധനാ രീതികളുടെ പട്ടിക ഉണ്ട്. പുതിയ പരിശോധനാ രീതികൾ വേഗത്തിനായി ഇവിടെ സംരക്ഷിക്കാവുന്നു. ക്രെഡിറ്റ് കാർഡ് നമ്പറുപോലുള്ള നിങ്ങളുടെ പരിശോധനാ വിവരങ്ങൾ ലിറ്റററി യൂണിവേഴ്‌സ് സംരക്ഷിക്കുന്നില്ലെന്ന് നിങ്ങളെ അറിയിക്കുന്നു.',
  'payment.history.title': 'പരിശോധന ചരിത്രം',
  'payment.history.tab': 'History',
  'payment.history.none': "We don't have any records of past payments.",
  'plans.options.aria': 'Payment options',
  'plans.options.creditCard': 'Credit Card',
  'plans.options.creditCardNew': 'New Credit Card',
  'plans.options.existingCards': 'Saved Credit Cards',
  'plans.purchase.action': 'Subscribe',
  'plans.purchase.title': 'Buy Literary Universe subscription',
  'payment.creditCard.details': 'Credit Card details',
  'payments.zipcode.label': 'ZIP code',
  'payment.card.name': 'Name on card',
  'plans.subscribed.thankYou': 'Thank you for supporting Literary Universe',
  'plans.subscribed.periodEnd': 'നിങ്ങളുടെ സബ്സ്ക്രിപ്ഷൻ വായിക്കൽ ചെയ്യാൻ പരിശോധിക്കുന്നു: {end, date, medium}',
  'plans.subscribed.renewal': `അവസാനത്തെ നിങ്ങളുടെ സബ്സ്ക്രിപ്ഷൻ ഓട്ടോമാറ്റിക്കല്ലെങ്കിൽ {method, select,
    charge_automatically {പുതുക്കപ്പെടും}
    send_invoice {ഒരു ഇൻവോയിസ് പണം പിടിച്ച് പുതുക്കപ്പെടും}
    other {}
  }.`,
  'plans.subscribed.paymentMethod':
    'Your default payment method will be used to pay for the next cycle.',
  'plans.subscribed.status': `അംഗത്വം നില: {status, select,
    active {സജീവമാണ്}
    incomplete {ഒന്നും കാണുന്നില്ല}
    incomplete_expired {ഒന്നും കാണുന്നില്ല}
    trialing {ട്രയൽ}
    past_due {നിങ്ങളുടെ പേയ്മെന്റ് കാണുന്നില്ല}
    cancelled {റദ്ദാക്കിയിട്ടുണ്ട്}
    unpaid {അപേക്ഷിക്കാത്തവ}
    expired {കാലാവധിയായിരിക്കുന്നു}
    other {}
  }`,
  'payments.changeMethod': 'Change payment method',
  'plans.subscribed.willNotRenew': ', will not renew at the end of the cycle.',
  'plans.cancel.button': 'Cancel subscription',
  'plans.cancel.question':
    'Are you sure you want to cancel your subscription? Your subscription will remain active till the end of the billing cycle.',
  'plans.cancel.confirmationButton': 'Yes, I want to cancel my subscription',
  'plans.cancel.failed':
    'Apologies, we were unable to cancel your plan at this moment. Please try again later.',
  'plans.cancel.restoreBtn': 'Restore subscription',
  'plans.cancel.restoreFailed':
    'Apologies, we were unable to restore the subscription at this moment. Please try again later.',
  'plans.change.button': 'Change subscription',
  'plans.change.explanation':
    'Select the plan you want to upgrade or downgrade to. You will be billed the difference in price immediately.',
  'plans.change.failed':
    'Apologies, we were unable to change your plan at this moment. Please try again later.',
  'plans.change.upgrade': 'Upgrade',
  'payments.disclaimer':
    'പേയ്മെന്റ് വിവരങ്ങൾ അനുവദിക്കുന്നതും പ്രോസസ്സുചെയ്യുന്നതും സ്ട്രൈപ്പ് വഴിയാണ്. ഞങ്ങൾക്ക് സെർവറുകൾക്ക് എന്തെങ്കിലും ക്രെഡിറ്റ് കാർഡ് അല്ലെങ്കിൽ ബാങ്ക് അക്കൗണ്ട് വിവരങ്ങൾ സംരക്ഷിക്കരുത്.',
  'buy.payment.optionsSelect': 'Select payment option',
  'payments.card.number': 'Card number',
  'payments.card.expire': 'Valid Thru',
  'payments.card.cvv': 'CVV',
  'payments.card.info': '{cardType} card, ending in {ending}',
  'payments.none': 'സംഭരിച്ച പണം ഓപ്ഷനുകൾ കണ്ടെത്തിയില്ല.',
  'payments.none.cards': 'No cards were found.',
  'payments.added': 'പുതിയ പണം ഓപ്ഷൻ വിജയകരമായി ചേർത്തിട്ടുണ്ട്.',
  'payments.delete.confirm': 'നിങ്ങൾ ഉറപ്പാക്കാൻ താൽപ്പര്യപ്പെടുന്നുണ്ടോ {cardType}, അവസാനിക്കുന്നത് {ending}?',
  'payment.invoice.status': 'ഇൻവോയിസ് അവസ്ഥ',
  'payment.invoice.statusMessage': `{status, select,
    draft {ഡ്രാഫ്റ്റ്}
    open {തുറന്നുവഴിയാക്കുക}
    paid {പണം പിടിച്ചെടുക്കുക}
    uncollectible {പണം അടയാളപ്പെടാൻ അയാളുടെ കഴിവില്ലാത്തതാണ്}
    void {വെയിഡ്}
    other {}
  }`,
  'payment.invoice.id': 'ഇൻവോയിസ് ഐഡി',
  'payment.invoice.pdf': 'ഇൻവോയിസ് പിഡിഎഫ്',
  'payment.invoice.total': 'മൊത്തം',
  'payment.invoice.what': 'ഇൻവോയിസ് വിവരണം',
  'payment.stripe.powered': 'പ്രാമുഖ്യം നൽകുന്നത് ',
  'payment.listMethods.card': 'കാർഡുകൾ',
  'payment.expiresAt': 'അവസാനിക്കുന്നത് {month}/{year}',
  'payment.card.details': `{type, select,
    credit {ക്രെഡിറ്റ് കാർഡ്}
    debit {ഡെബിറ്റ് കാർഡ്}
    other {കാർഡ്}
  } അവസാനിക്കുന്നത് {last4}`,
  'payment.options.default': 'സ്വതേ അവസ്ഥിതം',
  'payment.method.add': 'പുതിയ പണം രീതി ചേർക്കുക',
  'payment.methods.adding': 'നിങ്ങളുടെ പുതിയ പണം രീതി നിങ്ങളുടെ അക്കൗണ്ടിലേക്ക് ചേർത്തിട്ടുണ്ട്.',
  'payment.methods.delete': 'പണം രീതി മാറ്റാൻ നിർദ്ദേശിക്കുന്നു',
  'payment.upgradePlan.option': '{plan} - {price}',
  'payment.methods.webMonetization': 'വെബ് മോണെറ്റൈസേഷൻ',
  'settings.webMonetization.placeholder': 'നിങ്ങളുടെ പേയ്മെന്റ് പോയിന്റർ',
  'payment.webMonetization.explained':
    'വെബ് മോണെറ്റൈസേഷൻ വെബ് പേയ്മെന്റുകളിലേക്ക് ഒരു പ്രസ്താവിത നിർദ്ദേശമാണ്. ഇവിടെ നിങ്ങളുടെ പേയ്മെന്റ് പോയിന്റർ ചേർക്കാൻ കഴിയും. അത് ശേഖരിക്കപ്പെടുന്നത് ശേഖരിച്ച് നിങ്ങളുടെ പ്രൊഫൈൽ പേജ്, ബ്ലോഗ്, യൂണിവേഴ്സുകൾ ഒപ്പം കഥകളിൽ ഉപയോഗിക്കാൻ ഉപയോഗിക്കാവുന്നതാണ്.',
  'payment.webMonetization.learnMore': 'വെബ് മോണെറ്റൈസേഷൻ കുറിച്ച് കൂടുതൽ അറിയുക',
  'plans.subscription.settingsAria': 'Subscription settings options',
  'plans.buttonStatus.submitting': 'Submitting your request...',
  'plans.button.manageOnStripe': 'Manage Billing on Stripe',
  'plans.call.signUpBottom': 'Sign-up for a free Literary Universe account and start creating!',
  'plans.button.seeComparison': 'Find out more about plans and pricing on our comparison page!',
  'plans.button.chooseThisSubscription': 'Select this plan',
  'plans.button.chooseFree': 'Start for free',
  'plans.features.allInPreviousTiers': 'Everything in previous tier',
  'plan.button.chooseNow': 'Choose your Literary Universe plan now',
  'plans.subscription.manage': 'Manage subscription',
  'plans.taxMayApply': '* VAT and other local taxes may apply',
  'plans.faq.crypto': 'Will you offer crypto payments?',
  'plans.faq.cryptoAnswer':
    'Yes, we are working on it. We are big fans of Web3 we are looking for ways to integrate Web3 into the platform in the near future. We currently allow for login with Web3 and we are looking to expand on this offering especially when it comes to providing compensation to creators and giving creators the ability to choose all the possible payment options that they want to accept.',
  'plans.subscribed.updating':
    'It might take a bit to update your your subscription information. Thank you for your patience!',
  'payments.methods.webMonetization': 'വെബ് മോണെറ്റൈസേഷൻ',
  'stripeConnect.settings.title': 'പേയൗട്ട് ക്രമീകരണങ്ങൾ',
  'stripeConnect.settings.intro':
    'If you want to receive funds or sell through Literary Universe you need to create Stripe account via the button bellow.',
  'stripeConnect.settings.connectAccount': 'സ്ട്രൈപ്പ് അക്കൗണ്ട് സജ്ജമാക്കുക',
  'stripeConnect.settings.ongoing':
    'നിങ്ങളുടെ അക്കൗണ്ടിനെ പറ്റി സ്ട്രൈപ്പിലേക്ക് പിന്തുണയ്ക്കാൻ ഞങ്ങൾ കാത്തിരിക്കുന്നു. ഇത് അല്ലെങ്കിൽ നിങ്ങളുടെ കാത്തിരിപ്പ് നീക്കാൻ നിങ്ങൾക്ക് വളരെ നേരം എടുക്കും. ഇതിനാൽ നിങ്ങൾക്ക് താഴെയുള്ള ബട്ടൺ ക്ലിക്ക് ചെയ്യുക',
  'stripeConnect.settings.finished':
    'സ്ട്രൈപ്പ് കണക്റ്റ് അക്കൗണ്ട് ഉപയോഗിച്ച് പണം ലഭിക്കാൻ നിങ്ങൾ പൂർത്തിയാക്കിയിരിക്കുന്നു',
  'stripeConnect.settings.balance': 'ബാലൻസ്',
  'stripeConnect.settings.balanceAvailable': 'ലഭ്യമായ ബാലൻസ്',
  'stripeConnect.settings.balancePending': 'കാത്തിരിപ്പിലുള്ള ബാലൻസ്',
  'stripeConnect.settings.recentTransfers': 'പുതിയ മാറ്റങ്ങൾ',
  'stripeConnect.transfers.amount': 'തുക',
  'stripeConnect.transfers.reason': 'കാരണം',
  'payment.settings.donations': 'സംഭാവന ക്രമീകരണങ്ങൾ',
  'plans.fanClub.heading': 'ഫാൻ ക്ലബുകൾ',
  'payment.donations.enable': 'സംഭാവനകൾ പ്രാപ്തമാക്കുക',
  'payment.donations.suggestedAmount': 'ശുപാർശിത തുക',
  'payment.donations.amount': 'തുക',
  'payment.donations.preferredCurrency': 'പ്രിയപ്പെട്ട നാണയം',
  'payment.donations.currency': 'നാണയം',
  'payment.donations.stripeDescription': '{username}എന്നിവരേയും സംഭാവന ചെയ്യുക',
  'payment.donations.support': '{username}പിന്തുണയ്ക്കുക',
  'payment.donations.donate': 'സംഭാവന ചെയ്യുക',
  'payment.donations.donationModal': 'സംഭാവന സ്ഥിരീകരണം',
  'payment.donations.payNowUSD':
    '{donation, number, ::compact-short currency/USD} നേടുക {username}എന്നിവരേയും സംഭാവന ചെയ്യുക',
  'payment.donations.payNowJPY':
    '{donation, number, ::compact-short currency/JPY} നേടുക {username}എന്നിവരേയും സംഭാവന ചെയ്യുക',
  'payment.donations.payNowEUR':
    '{donation, number, ::compact-short currency/EUR} നേടുക {username}എന്നിവരേയും സംഭാവന ചെയ്യുക',
  'payment.donations.payNowCZK':
    'Donate {donation, number, ::compact-short currency/CZK} to {username}',
  'payment.donations.payNowGBP':
    'Donate {donation, number, ::compact-short currency/GBP} to {username}',
  'payment.donations.thankYouTitle': 'Thank you for your donation to {username}',
  'payment.donations.thankYouContinue':
    'Thank you for supporting {username}, we hope that you will continue enjoying their work and consider supporting them again in the future.',
  'payment.success': 'Payment succeeded!',
  'payment.processing': 'Your payment is processing.',
  'payment.failed': 'Your payment was not successful, please try again.',
  'payment.generalError': 'Something went wrong.',
  'payment.unexpectedError': 'Something went wrong.',
  'payments.transferType.donation': 'Donation',
  'payment.donation.backToProfile': "Back to {username}'s profile",
  'payment.author.requirementStripeConnect':
    'Before we can process any financial transactions you need to create and connect your Stripe Connect account.',
  'payment.author.requirementPublishWork':
    'Before you continue, please publish at least one work (story or universe).',
  'payment.author.createWork': 'Create a new work',
  'payment.author.directivesTitle': 'Recommendations',
  'payment.author.directivesExplained':
    'We have some additional recommendations that you should fulfill before enabling payment services to increase your chance of success.',
  'payment.author.directivesStoryPublished':
    'Publish at least one story for free to showcase your craft.',
  'payment.author.directivesFreeStoryFinished':
    "Finish at least one free story to show your readers that you won't abandon them in the middle of the journey.",
  'payment.universe.directivesUniversePublished': 'Publish your universe',
  'payment.universe.directivesEncFull':
    'Fill up your encyclopedia, have at least 30 entries! Better if you are attacking the 100 margin.',
  'payment.universe.haveFans':
    'നിങ്ങൾക്ക് സ്പോൺസർ ചെയ്യാൻ ആഗ്രഹിക്കുന്നവർ അവരുടെ പ്രവർത്തനത്തിൽ ആഗ്രഹിക്കുന്നവരാണെന്ന് അറിയാം. 100 ആരാധകരുടെ യൂണിവേഴ്സ് അവരുടെ ലൈബ്രറിയിൽ ചേർക്കുന്നത് ആരംഭ രേഖയാണ്.',
  'payment.settings.yourFanClub': 'നിങ്ങളുടെ ആരാധക ക്ലബ്',
  'payment.settings.newWork': 'ഒരു പുതിയ പ്രവർത്തനം സൃഷ്ടിക്കുക',
  'payment.amount.withNet': `{amount} (നെറ്റ്: {netAmount})`,
  'payment.fanClub.intro':
    'In this section you can modify your fan club settings. From subscription tiers to history of payments.',
  'payment.fanClub.tiers': 'Tiers',
  'payment.fanClub.tierName': 'ടിയർ പേര്',
  'payment.fanClub.createTier': 'ടിയർ ചേർക്കുക',
  'payment.fanClub.description': 'ടിയർ വിവരണം',
  'payment.fanClub.picture': 'ടിയർ ചിത്രം',
  'payment.fanClub.priceBreakdown':
    'സബ്സ്ക്രൈബർ അടയാളം പറയും: {amount}. പ്ലാറ്റ്‌ഫോം ഫീ ആണ് {fee}, നിങ്ങൾക്ക് ലഭിക്കും: {net}',
  'payment.feeNotice':
    'ദയവായി ശ്രദ്ധിക്കുക, ഇത് അംഗീകരിക്കുന്നത് അറിയാം എന്നതാണ് മികച്ചപ്രകടനം ആണ്, കസ്റ്റമർമാർക്ക് പരിശോധനാത്മക തരം അടയാളങ്ങൾക്കും പേയ്‌ഗേറ്റിൽ നിന്നുള്ള അധിക ചാർജുകളിലേക്കുള്ള അടയാളങ്ങൾക്കും അനുസരിച്ച് വ്യത്യസ്തമാകാം.',
  'payment.fanClub.perks': 'പ്രയോജനങ്ങൾ',
}
